...
[r][emb exp='tf.covering.displayName'] protected [emb exp='game.curTarget.displayName']!
@wait time=300
...
; 読み込み済み
@image layer=e3 storage=screen_deepred visible=true
@layopt layer=e10 left=&'targetX - 400' top=&'targetBottom - 550' visible=true
@playse storage=fire1
@anime layer=e10
@anime_fadein layer=e3,e10
@wait time=1480
@anime_fadeout layer=e3,e10
@stopanime layer=e10
@return
...
[endif]
@image layer=e3 storage=BE_darkcurtain left=0 top=0
...
[if exp='game.party.isWaterWalking()']
	[r]But couldn't jump due to being on a boat!
	@playse buf=2 storage=buzzer
	@notice text="<y>Can't jump while boat is moving!"
	@btmes off
	@return
[endif]
...
[else]
	@anime_number layer=e3 left=350 top=400 text="ミス"
[endif]
@return
...
@eval exp='game.cutOutFigure("alternate")'
@eval exp='tf.i = 0'
@command type=long title="Do you seek a miracle?"
*_spell_mahaman_loop
...
@eval exp='tf.i = 0'
@command type=long title="What would you like?" cancel=true
*_spell_iharon_loop
@select text=&'game.curFighter.act.choice[tf.i].order' exp=&'"tf.num = " + tf.i'
@eval exp='tf.i ++'
...
[if exp='game.curFighter.act.choice[tf.num].range == 1']
	@waitchoosechara text="Select a target"
...
@wait time=500
[texton w=0]
[ミレディ f=01]「I'll use Anti-Magic～!」[k]
[textoff]
@wait time=500
@btmes show
@se f=shakin3
